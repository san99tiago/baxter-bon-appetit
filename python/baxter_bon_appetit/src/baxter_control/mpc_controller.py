...
class MpcController:
    ...
    def __init__(self, N, show_results):
...
if __name__ == "__main__":
    ...
    current_orientation = transf.Transformation(0, 0, 0, [0, 0, 0]).get_fixed_angles_from_tm(tm_current)
    ...
    new_orientation = transf.Transformation(0, 0, 0, [0, 0, 0]).get_fixed_angles_from_tm(tm_new)

    print("----CURRENT CARTESIAN----")
    print(np.concatenate([current_position, current_orientation], axis=0).reshape(6, 1))
    print("----NEW CARTESIAN----")
    print(np.concatenate([new_position, new_orientation], axis=0).reshape(6, 1))
